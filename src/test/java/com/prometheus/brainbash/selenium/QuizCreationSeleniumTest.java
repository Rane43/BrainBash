...
class QuizCreationSeleniumTest {
	...
	void successfullyCreateAGame() throws InterruptedException {
		...
		// Get all quiz cards
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id(MODAL_ID))); // Modal disappears
        ...
        for (WebElement card : allCards) {
            ...
            if (headerText.contains(newTitle) && paragraphText.contains(newDescription)) {
                cardFound = true;  // If any card matches the condition
                break;  // Exit the loop if we find a matching card
            }
        }
